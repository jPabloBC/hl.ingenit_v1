...
const reservationStatuses = [
  { value: 'confirmed', label: 'Reservada', color: 'bg-green-100 text-green-800' },
...
export default function ReservationsManagement() {
  ...
  // Function to get business date (same as front-desk and sales)
  ...
  const loadReservations = async () => {
    ...
    try {
      // Get business country for date calculation
      ...
      if (filterDate === 'today') {
        ...
        query = query.gte('check_in_date', startOfWeek.toISOString().split('T')[0])
        ...
        query = query.gte('check_in_date', startOfMonth.toISOString().split('T')[0])
      ...
      const transformedData = (data || []).map(reservation => ({
        ...
        room_number: reservation.hl_rooms?.[0]?.room_number || '',
        room_type: reservation.hl_rooms?.[0]?.room_type || '',
        floor: reservation.hl_rooms?.[0]?.floor || 0,
        primary_guest_id: reservation.id || '',
        ...
        room_price_per_night: reservation.room_price_per_night,
        total_amount: reservation.total_amount,
        special_requests: reservation.special_requests,
        status: reservation.status,
        created_at: reservation.created_at,
        all_guests: [] // We'll add this later if needed
      }));
  ...
  const loadRoomsWithReservations = async () => {
    ...
    try {
      ...
      // First, get all rooms for this business
      ...
      // Then, get all reservations for this business
      ...
      const roomsWithReservations = (roomsData || []).map(room => {
        const roomReservations = (reservationsData || []).filter(
        ...
        const currentReservation = roomReservations.find(
          res => res.status === 'checked_in' || (res.status === 'confirmed' && res.check_in_date <= today)
        );
        
        // Solo la próxima reserva más cercana (la primera después de hoy)
        const upcomingReservations = roomReservations
          .filter(res => res.status === 'confirmed' && res.check_in_date > today)
          .sort((a, b) => new Date(a.check_in_date).getTime() - new Date(b.check_in_date).getTime())
          .slice(0, 1); // Solo la primera (más cercana)
        
        const pastReservations = roomReservations.filter(
          res => new Date(res.check_out_date) < new Date()
        );
        ...
        return {
          ...
          reservations_count: roomReservations.length,
      ...
      setRoomsWithReservations(roomsWithReservations);
    } catch (error) {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      if (reservationsError) {
        console.error('Error loading reservations for stats:', reservationsError);
        return;
      }
      ...
      if (roomsError) {
        console.error('Error loading rooms for stats:', roomsError);
        return;
      }
      ...
      const stats = {
        ...
        today_checkins: reservations.filter(r => r.check_in_date === today && r.status === 'confirmed').length,
        today_checkouts: reservations.filter(r => r.check_out_date === today).length,
        ...
        total_revenue: reservations.reduce((sum, r) => sum + (r.total_amount || 0), 0),
        occupancy_rate: rooms.length > 0 ? Math.round((reservations.filter(r => r.status === 'checked_in').length / rooms.length) * 100) : 0,
      ...
      <div className="w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray4">Reservadas</p>
                          ...
                          <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3">
                            <div className="flex items-center space-x-2 mb-2">
                              ...
                              <span className="font-medium text-red-800">
                                {room.current_reservation.status === 'checked_in' ? 'Ocupada' : 'Reservada'}
                              </span>
                            </div>
                            <div className="text-sm text-red-700">
                              <div className="font-medium">{room.current_reservation.primary_guest_name}</div>
                              <div>{room.current_reservation.primary_guest_email}</div>
                              <div>
                              ...
                              <div className="mt-1">
                                <span className={`px-2 py-1 rounded-full text-xs ${
                                  room.current_reservation.status === 'checked_in' 
                                    ...
                                    : room.current_reservation.status === 'confirmed'
                                    ? 'bg-yellow-100 text-yellow-800'
                                    : getStatusColor(room.current_reservation.status)
                                }`}>
                                  {room.current_reservation.status === 'checked_in' 
                                    ? 'Check-In' 
                                    : room.current_reservation.status === 'confirmed'
                            ...
                            <div className="space-y-1">
                              {room.upcoming_reservations.slice(0, 2).map((reservation: any, idx: number) => (
                                <div key={idx} className="text-xs text-blue-700">
                                  <div className="font-medium">{reservation.primary_guest_name}</div>
                                  <div>
                    ...
                    <tr key={reservation.reservation_id} className="hover:bg-gray10">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-blue1">
                          <div>Check-in: {reservation.check_in_date}</div>
                          <div>Check-out: {reservation.check_out_date}</div>
                        </div>
